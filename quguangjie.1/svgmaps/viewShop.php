...
if(isset($_POST['newComm']))
{
	...
	$newRanking = ($row['ranking'] * $row['commentCount'] + $ranking * $axis)/($row['commentCount']+$axis);
	$newCommentCount = $row['commentCount'] + $axis;
	$query = $_POST['mapName']=="sjtuShop"?"update sjtuShop set ranking = '$newRanking',commentCount = '$newCommentCount' where ID = '$idNum'":"";
	$result = $link->query($query);
	$shopName = $row['shopName'];
	$userIdNum = $_SESSION['userId'];
	$query = "select name from userInfo where ID = '$userIdNum'";
	$result = $link->query($query);
	if($row1 = $result->fetch_assoc()) {
		$userName = $row1['name'];
	}
	$time=time();
	$query = $_POST['mapName']=="sjtuShop"?"insert into comment(userName,shopName,comments,userId,shopId,shopMap,ranking,time)
								values('$userName','$shopName','$comments','$userIdNum','$idNum','$shopMap','$ranking','$time')":"";
	$link->query($query);
}
/*
...
<div id="comments">
	...
	<?php 
		...
		while($row = $result->fetch_assoc()) {
			if($row['Remark.content']!="")
			{
				echo "<div class = \"events\" >" . $row['User.name']. " 评论道：" . $row['Remark.content'] . "</div>"; 
				echo "<div class = \"events\" >打分 " . $row['Remark.score'] . " 分</div>";
				echo "<br/>";
			}
			else
			{
				echo "<div class = \"events\" >" . $row['User.name']. " 打分：" . $row['Remark.score'] . " 分</div>";
				echo "<br/>";
			}
		}
		?>
